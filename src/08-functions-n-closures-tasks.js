...
function logger(func, logFunc) {
  return function a(...args) {
    const funcArgs = JSON.stringify(args);
    logFunc(`${func.name}(${funcArgs.slice(1, funcArgs.length - 1)}) starts`);
    const res = func(...args);
    logFunc(`${func.name}(${funcArgs.slice(1, funcArgs.length - 1)}) ends`);
    return res;
  };
}
